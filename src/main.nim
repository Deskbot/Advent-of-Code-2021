...
proc hasWon(board: Board): bool =
    # rows
    for line in board.checks.lines():
        if all(line, proc (b: bool): bool = b == true):
            return true

    # cols
    for col in board.checks.rows():
        if all(col, proc (b: bool): bool = b == true):
            return true
